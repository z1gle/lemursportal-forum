                                    ...
                                    <form id="form-login" class="login-form" action="javascript:login();" name="loginForm" method="POST">
                                        <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
                                            ...
                                            <div class="input-group" style="width: 100%;">
                                                <input id="pwd" class="pwd"  style="border: 1px solid #ccc; border-right: none;" type="password" name="password" placeholder="${pwdPlaceholder}"/>
                                                <span class="input-group-btn">
                                                    <div style="height: 42px; margin: 0px; width: 40px; background-color: white; 
                                                         color: #ccc; margin-top: -14px; padding: 10px; border: 1px solid #ccc; 
                                                         border-radius: 0px; border-left: none;" 
                                                         type="button" class="btn btn-default">
                                                        <i id="pwd-eye" class="fa fa-eye" style="padding-top: 3px;" onclick="showPassword()"></i>
                                                    </div>
                                                </span>
                                            </div>
                                                ...
                                                <script>
                                                    ...
                                                    function showPassword() {
                                                        if($('#pwd')[0].type == 'password') {
                                                            $('#pwd-eye').removeClass();
                                                            $('#pwd-eye').addClass('fa fa-eye-slash');
                                                            $('#pwd')[0].type ='text';
                                                        } else if ($('#pwd')[0].type == 'text') {
                                                            $('#pwd')[0].type = 'password';
                                                            $('#pwd-eye').removeClass();
                                                            $('#pwd-eye').addClass('fa fa-eye');
                                                        }
                                                    }
