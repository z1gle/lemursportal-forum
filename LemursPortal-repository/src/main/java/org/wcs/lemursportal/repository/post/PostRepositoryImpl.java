...
//		jpql.append(" group by p.parentId order by nbResponse desc ");
    ...
    private void populateNombreDeVue(Map<Integer, TopQuestion> topQuestionMap) {
        if (topQuestionMap == null || topQuestionMap.isEmpty()) {
            return;
        }
    ...
    public void insert(Post p) {
        ...
        if (p.getDocuments() != null) {
            for (int i = 0; i < p.getDocuments().size(); i++) {
                p.getDocuments().get(i).setPostId(p.getId());
...
//                em.persist(p.getDocuments().get(i));
    ...
    public Page<Post> getYoutubeVideo(Pageable pageable) {
        ...
        StringBuilder jpqlCount = new StringBuilder("select count(p.id) from Post p where p.uriYoutube is not null and (p.deleted=:notDeleted or p.deleted is null)");
        TypedQuery<Post> query = em.createQuery(jpql.toString(), Post.class);
        query.setParameter("notDeleted", false);
    ...
    public void update(Post post) {
        ...
        if (post.getDocuments() != null) {
            for (int i = 0; i < post.getDocuments().size(); i++) {
                post.getDocuments().get(i).setPostId(post.getId());
                em.persist(post.getDocuments().get(i));
            }
        }
    }
