...
//		}
    ...
    @Override
    public Page<TopThematique> findTopThematique(Pageable pageable, boolean document) {        
        String qry = "select t.* from thematique t order by t.id asc";
        Query query = em.createNativeQuery(qry, Thematique.class);
        if (pageable != null) {
            query.setFirstResult(pageable.getOffset());
            query.setMaxResults(pageable.getPageSize());
        }
        List<Thematique> thematiques = query.getResultList();
        qry = "select count(a.id_topic) from thematique t left join association_metadata_topic a on t.id = a.id_topic group by t.id order by t.id asc";
        query = em.createNativeQuery(qry);
        if (pageable != null) {
            query.setFirstResult(pageable.getOffset());
            query.setMaxResults(pageable.getPageSize());
        }
        List<BigInteger> nbDocument = query.getResultList();
        qry = "select count(m.thematique_id) from thematique t left join message m on m.thematique_id = t.id group by t.id order by t.id asc";
        query = em.createNativeQuery(qry);
        if (pageable != null) {
            query.setFirstResult(pageable.getOffset());
            query.setMaxResults(pageable.getPageSize());
        }
        List<BigInteger> nbPost = query.getResultList();
        
        List<TopThematique> valinyFarany = new ArrayList<>();
        for (int i = 0; i < thematiques.size(); i++) {
            TopThematique tt = new TopThematique();
            tt.setThematique(thematiques.get(i));
            tt.setNombreDocument(nbDocument.get(i).longValue());
            tt.setNombreQuestion(nbPost.get(i).longValue());
            valinyFarany.add(tt);
        }        
        return new PageImpl<>(valinyFarany, pageable, valinyFarany.size());
    }
}
