...
//    }
    ...
    public Page<Metadata> findAll(Pageable pageable, String type, Integer idThematique) {
        String qry = "select distinct d.* from metadata d join association_metadata_topic a on d.id = a.id_metadata where a.id_topic = :idTopic and d.type = :type";
        Query query = em.createNativeQuery(qry, Metadata.class);
    ...
    public Page<Metadata> findAll(Pageable pageable, String type, Integer idThematique, int orderByYear) {
        String qry = "select distinct d.* from metadata d join association_metadata_topic a on d.id = a.id_metadata where a.id_topic = :idTopic and d.type = :type order by d.year desc";
        if (orderByYear > 0) {
            qry += " order by d.year asc";
        } else if (orderByYear < 0) {
            qry += " order by d.year desc";
